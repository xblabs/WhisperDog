...
public class SilenceRemover {
    ...
    public static File removeSilence(File originalFile, float silenceThresholdRMS,
        ...
        try {
            ...
            // Warning for high silence ratio, but still process
            // User feedback: long pauses (thinking, looking something up) should not cause
            // the recording to be dropped. Better to allow false positives through than
            // lose actual user dictation work.
            if (reductionPercent > 90.0f) {
                console.log("⚠ High silence ratio detected (>90%)");
                console.log("  This could indicate: long thinking pause, aggressive threshold, or quiet audio");
                console.log("  Processing anyway to preserve any speech content");
            }
