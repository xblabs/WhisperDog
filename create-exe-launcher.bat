@echo off
REM Create WhisperDog.exe launcher using Launch4j
REM This creates a proper .exe that can be pinned to taskbar

setlocal

echo WhisperDog EXE Launcher Builder
echo ================================
echo.
...
if not exist "WhisperDog.exe" (
    echo ERROR: WhisperDog.exe was not created!
    pause
    exit /b 1
)
...
echo.
echo ========================================
echo SUCCESS! WhisperDog.exe created!
echo ========================================
echo.
echo You can now:
echo 1. Run WhisperDog.exe directly
