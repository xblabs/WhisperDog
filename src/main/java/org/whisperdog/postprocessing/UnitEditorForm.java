...
import org.whisperdog.MainForm;
import org.whisperdog.Notificationmanager;
import org.whisperdog.ToastNotification;
import org.whisperdog.postprocessing.clients.OpenWebUIModelsResponse;
import org.whisperdog.postprocessing.clients.OpenWebUIProcessClient;
...
public class UnitEditorForm extends JPanel {
    ...
    public UnitEditorForm(ConfigManager configManager, MainForm mainForm, ProcessingUnit existingUnit) {
        ...
        // Bottom panel with done button and spawn pipeline button
        ...
        JButton spawnPipelineButton = new JButton("Save & Create Pipeline");
        spawnPipelineButton.setToolTipText("Save this unit and create a new pipeline with it as the only step");
        spawnPipelineButton.addActionListener(e -> saveAndCreatePipeline());
        bottomPanel.add(spawnPipelineButton);
    ...
    /**
     * Saves the unit and creates a new pipeline with this unit as the only step.
     * Useful for the common case of 1:1 unit-to-pipeline relationship.
     */
    protected void saveAndCreatePipeline() {
        ProcessingUnit unit = saveUnit();
        if (unit == null) {
            return;  // Validation failed
        }

        // Create a new pipeline with this unit
        Pipeline pipeline = new Pipeline();
        pipeline.uuid = UUID.randomUUID().toString();
        pipeline.title = unit.name;  // Use the unit name as pipeline title
        pipeline.description = "Pipeline for: " + unit.name;
        pipeline.enabled = true;

        // Add the unit reference to the pipeline
        ...
        Notificationmanager.getInstance().showNotification(ToastNotification.Type.SUCCESS,
            "Unit saved and pipeline '" + pipeline.title + "' created!");

        // Go to pipeline list
        mainForm.setSelectedMenu(3, 0);
        mainForm.showForm(new PipelineListForm(configManager, mainForm));
    }

    /**
     * Validates and saves the current unit data.
     * @return The saved ProcessingUnit, or null if validation failed
     */
    private ProcessingUnit saveUnit() {
        if (!validateData()) {
            return null;
        }
