# WhisperDog Windows Installation Guide

## Overview

WhisperDog provides multiple ways to package and distribute the application for Windows 10/11 users.

## Settings Persistence ✓

**Your settings are automatically saved!**
- Location: `%APPDATA%\WhisperDog\config.properties`
...
**Output:** `installer-output\WhisperDog-1.6.0.exe`
...
- ✅ Settings persist to `%APPDATA%\WhisperDog\`
...
**Output:** `WhisperDog-Portable.zip`

**Contents:**
- `WhisperDog.jar` - Application
- `WhisperDog.bat` - Launcher script
...
The CI/CD automatically creates `WhisperDog-Windows-with-jre-non-installer.zip` which includes:
- Application JAR
- Bundled Zulu JRE 17
...
1. Run `WhisperDog-1.6.0.exe`
2. Follow installation wizard
3. Launch from Start Menu
4. Settings auto-save to `%APPDATA%\WhisperDog\`

### If you provide the portable EXE:
1. Download `WhisperDog.exe`
...
1. Extract `WhisperDog-Portable.zip`
2. Double-click `WhisperDog.bat`
3. Settings auto-save to `%APPDATA%\WhisperDog\`

---

## How Settings Work

### ConfigManager (src/main/java/org/whisperdog/ConfigManager.java)
...
- `WhisperDog-Windows-Installer.exe` (jpackage)
- `WhisperDog-Windows-with-jre-non-installer.zip` (Launch4j + JRE)
- `whisperdog.deb` (Linux Debian package)
- `WhisperDog.AppImage` (Linux portable)
- `WhisperDog-macOS-Experimental.dmg` (macOS)
...
- WhisperDog requires Java 11+
- Installer builds require JDK 16+ (for jpackage)

### Settings not persisting
- Check `%APPDATA%\WhisperDog\` directory exists
...
- Check `whisperdog.ico` exists in `src/main/resources/`
...
- Main repo: https://github.com/xblabs/whisperdog-xb
- GitHub Actions workflows: `.github/workflows/build-workflow.yml`
- Build configuration: `pom.xml` (Maven)
- Settings manager: `src/main/java/org/whisperdog/ConfigManager.java`
